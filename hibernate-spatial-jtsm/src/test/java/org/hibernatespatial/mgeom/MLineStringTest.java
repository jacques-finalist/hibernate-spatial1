    ...
    */
    protected void setUp() throws Exception {
        ...
        for (int i = 0; i < mcoar.length; i++) {
            ...
            double z = Double.NaN; //JTS doesn't support operations on the z-coordinate value
...
//    }
    ...
    public void testGetClosestPoint() {
        ...
        try {
            ...
            //must return the very same coordinate if the coordinate is a coordinate of the line
            ...
            //check for cases of circular MGeometry: lowest measure should be return.
            ...
            mcexp = MCoordinate.create2dWithMeasure(mco1.x + offset * (mco2.x - mco1.x),
                    mco1.y + offset * (mco2.y - mco1.y), 0.0);
            ...
            MCoordinate mcin = MCoordinate.create2dWithMeasure(mco1.x + offset * (mco2.x - mco1.x) + delta,
                    mco1.y + offset * (mco2.y - mco1.y) + delta, 0.0);
    ...
    public void testGetCoordinateAtM() {
        ...
        try {
            ...
            //if m is value of a coordinate, the returned coordinate should equal that coordinate
            ...
            MCoordinate mcexp = new MCoordinate(mco1.x + offset * (mco2.x - mco1.x),
    ...
    */
    ...
    public void testGetMatCoordinate() {
        try {
            ...
            //if a coordinate of the geometry is passed, it should return exactly that m-value
            MCoordinate mco1 = (MCoordinate) arbitraryLine.getCoordinateN(elem1Indx);
            double m = arbitraryLine.getMatCoordinate(mco1, 0.00001);
            assertEquals(mco1.m, m, DoubleComparator.defaultNumericalPrecision());

            //check for a coordinate between mco1 and mco2 (neighbouring coordinates)
            MCoordinate mco2 = (MCoordinate) arbitraryLine.getCoordinateN(elem2Indx);
            double offset = Math.random();
            double expectedM = mco1.m + offset * (mco2.m - mco1.m);
            Coordinate mctest = new Coordinate(mco1.x + offset * (mco2.x - mco1.x),
                    mco1.y + offset * (mco2.y - mco1.y));
    ...
    public void testInterpolate() {
        ...
        // Internal coordinate measures are not defined, outer measures are 0-10, total 2d length is 10
        ...
        // Test Continuous case by interpolating with begin and end measures equal
    ...
    public void testGetCoordinatesBetweenNonStrict() {
        try {
            ...
            // in this case, a 2nd and 3rd coordinates are the same instance, giving one a duplicate internal measure
            // The state is monotone, and dynseg from measures 0-1 should yield the equivalent of a similar
            // Line where the duplicate coordinate does not exist.  In other words the duplicate is essentially ignored.
            MLineString nonStrictDupPointLine = mgeomFactory.createMLineString(new MCoordinate[]{mc0, mc1, mc1, mc2, mc3});
            MLineString strictLine = mgeomFactory.createMLineString(new MCoordinate[]{mc0, mc1, mc2, mc3});
            ...
            // Test non-strict sequence where all coordinate x,y positions are unique, but contains a
            // duplicate measure.  The measure sequence in this test s [0,1,1,3,4]
            MLineString nonStrictPointLine = mgeomFactory.createMLineString(new MCoordinate[]{mc0, mc1, mc2_1, mc3, mc4});
