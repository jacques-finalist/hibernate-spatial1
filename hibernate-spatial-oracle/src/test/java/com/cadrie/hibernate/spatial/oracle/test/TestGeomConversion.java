...
import com.cadrie.hibernate.spatial.oracle.test.model.TestGeom;
 ...
 * @author Karel Maesen
 * 
 *The test geometries are those from the Oracle Spatial
 *User's Guide and Reference, section 2.5, Example 2-13
 *
 */
public class TestGeomConversion{
	...
	geom = geomFactory.createPolygon(geomFactory.createLinearRing(coordinates), null);
	...
	try {
	    ...
	    if (! isArc){
		assertTrue ("Geometries differ for case#: " + id , expected.get(id).equalsExact(geom));
	    } else {
		//only check if coordinates of expected are on the retrieved geometry
		MultiPoint mpco = (MultiPoint)expected.get(id);
		for (int i = 0; i < mpco.getNumPoints(); i++){
		    assertTrue("Arc coordinate not on geometry for case# " + id + "(point : " + mpco.getGeometryN(i) + "):", geom.isWithinDistance(mpco.getGeometryN(i), 0.03));
		}
	    }
	    return geom;
	} finally {
