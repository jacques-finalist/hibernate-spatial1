...
import com.cadrie.hibernate.spatial.SpatialAnalysis;
import com.cadrie.hibernate.spatial.SpatialDialect;
import com.cadrie.hibernate.spatial.SpatialRelation;
import com.vividsolutions.jts.geom.Geometry;

public class OracleSpatial9Dialect extends Oracle9Dialect implements
	SpatialDialect {
	 ...
	 * @param geomArrays indicates which argument places are occupied by sdo_geometries
	 */
	public WrappedOGCFunction (String name, Type type, boolean[] geomArrays){
	    super(name, type);
	...
	public String render(List args, SessionFactoryImplementor factory){
	    ...
	    buf.append(")");
	    return (getType().getReturnedClass() == Geometry.class) ? buf.append(".geom").toString() : buf.toString();
	}
	...
	public SpatialRelateFunction(String name, int relation) {
	...
	public String render(List args, SessionFactoryImplementor factory)
	    ...
	    return isOGCStrict() ? getOGCSpatialRelateSQL((String) args
		    .get(0), (String) args.get(1), this.relation)
	...
	public SpatialAnalysisFunction(String name, Type returnType, int analysis){
	    super(name, returnType);
	    this.analysis = analysis;
	}
	
	public String render(List args, SessionFactoryImplementor factory){
	    return isOGCStrict() ? getSpatialAnalysisSQL(args,this.analysis, false) :
	...
	//registerFunction("asbinary", new StandardSQLFunction("SDO_UTIL.TO_WKBGEOMETRY"));
	...
	registerFunction("boundary", new WrappedOGCFunction("OGC_BOUNDARY",
		new CustomType(SDOGeometryType.class, null), new boolean[]{true}));
	...
	registerFunction("buffer", new SpatialAnalysisFunction("buffer", new CustomType(SDOGeometryType.class, null), SpatialAnalysis.BUFFER));
	registerFunction("convexhull", new SpatialAnalysisFunction("convexhull", new CustomType(SDOGeometryType.class, null), SpatialAnalysis.CONVEXHULL));
	registerFunction("difference", new SpatialAnalysisFunction("difference", new CustomType(SDOGeometryType.class, null), SpatialAnalysis.DIFFERENCE));
	registerFunction("intersection", new SpatialAnalysisFunction("intersection", new CustomType(SDOGeometryType.class, null), SpatialAnalysis.INTERSECTION));
	registerFunction("symdifference", new SpatialAnalysisFunction("symdifference", new CustomType(SDOGeometryType.class, null), SpatialAnalysis.SYMDIFFERENCE));
	registerFunction("geomunion", new SpatialAnalysisFunction("union", new CustomType(SDOGeometryType.class, null), SpatialAnalysis.UNION));
	//we rename OGC union to geomunion because union is a reserved SQL keyword. (See also postgis documentation).
	...
	//	        String pureColumnName = columnName.substring(columnName.lastIndexOf(".")+1);
	//	        buffer.append("\"" + pureColumnName.toUpperCase() + "\"");
	buffer.append(arg1);
	buffer.append(",").append(arg2).append(",'mask=" + mask + "')").append(
		negate ? " !" : " ").append("= 'TRUE' ");
	...
	return isOGCStrict() ? getOGCSpatialRelateSQL(columnName, "?",
	...
	return isOGCStrict() ? getOGCSpatialAnalysisSQL(args, spatialAnalysisFunction) : getNativeSpatialAnalysisSQL(args, spatialAnalysisFunction);
    }
	...
	case SpatialAnalysis.UNION:
	    ...
	    break;
	default:
	    throw new IllegalArgumentException("Unknown SpatialAnalysisFunction ("
